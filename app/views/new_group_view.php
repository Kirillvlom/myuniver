			...
			<div class='row-1 padding-0 clearfix profile '>
				<div class='row  clearfix'>
					<div class='row-all flex-align-center clearfix'>
						<span class='title-span'>Добавить новую группу</span>
					</div>
				</div>
				<div class='row-all  clearfix'>
					<form action="/new/groupAdd" method="post">
						<div class="row clearfix">
							<div class="row-3">Название группы</div>
							<div class="row-1"><input type="text" class='textarea-chat' name='new_name_group' placeholder="Название" ></div>	
						</div>

						<div class="row clearfix ">
							<div class="row-3">Менеджер</div>
							<div class="row-1">
								
								<select name="new_id_manager" id="">
									<?

									for ($i=0; $i <  count($data["arrayManager"]); $i++) { 
										$userinfo = $db -> db_query_param_where_column("users", "id_user", $data["arrayManager"][$i]["id_user"], "id_user, name_user, surname_user, patronymic_user ");

										echo "<option value='".$data["arrayManager"][$i]['id_manager']."'>".$userinfo[0]['surname_user']." ".$userinfo[0]['name_user']." ".$userinfo[0]['patronymic_user']."</option>";
									}
									?>
								</select>
							</div>
								...
								<div class="row-2">
									...
									<div class="row-list-attached-students margin-10-0  " id='student_workspace'>
										...
										for ($i=0; $i < count($data['arrayStudens']) ; $i++) {
											...
											if (!$data['arrayStudens'][$i]['id_group']) {
												echo "

												<input type='checkBox' class='arrayUser_checkBox' name='arrayUser[]' value='".$data["arrayStudens"][$i]['id_student']."'>".$userinfo[0]['surname_user']." ".$userinfo[0]['name_user']." ".$userinfo[0]['patronymic_user']." <br>";
											}
