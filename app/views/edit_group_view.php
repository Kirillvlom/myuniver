			...
			<div class='row-1 padding-0 clearfix profile '>
				<div class='row  clearfix'>
					<div class='row-all flex-align-center clearfix'>
						<span class='title-span'>Удалить участников из группы</span>
					</div>
				</div>
				<div class='row-all  clearfix'>
					<form action="/edit/groupDeleteUser/<?echo $data["id_group"]?>" method="post">
						<div class="row clearfix">
									...
									<div class="row-list-attached-students margin-10-0  " id='student_workspace'>
										...
										/*for ($i=0; $i < count($data['arrayStudens']) ; $i++) {
											...
											if ($data['arrayStudens'][$i]['id_group'] AND $data['arrayStudens'][$i]['id_group'] == $data["id_group"] ) {
												...
												<input type='checkBox' checked='checked'  class='arrayUser_checkBox' name='arrayUser[]' value='".$data['arrayStudens'][$i]["id_student"]."'>".$userinfo[0]['surname_user']." ".$userinfo[0]['name_user']." ".$userinfo[0]['patronymic_user']." <br>";
						...
						<div class="row clearfix ">
							<div class="row-1"> <a href="/edit/add_student/<?echo $data["id_group"]?>">добавить участников</a></div>
							<div class="row-3"><input type="submit" class="new-message float-right" value="Удалить">
							</div>
						</div>
