			...
			<div class='row-1 padding-0 clearfix profile '>
				<div class='row  clearfix'>
					<div class='row-all flex-align-center clearfix'>
						<span class='title-span'>Оценки ВКР</span>
					</div>
				</div>
				<div class='row-all  clearfix'>
					<form action="/groups/seveEvaluation/" method="post" >
						<?
						$db = new Db();
						...
						for ($i=0; $i < count($data); $i++) { 
							...
							if ($data[$i][0]["evaluation_vkr"]) {
								echo "					<div class='row clearfix'>
								<div class='row-3'>".$user_FIO[0]["surname_user"]." ".$user_FIO[0]["name_user"]." ".$user_FIO[0]["patronymic_user"]."</div>
								<div class='row-1'><input type='text' class='textarea-chat' disabled  placeholder='Оценка ВКР' value='".$data[$i][0]["evaluation_vkr"]."' ></div>	
								</div>";
							}else{
								echo "						<div class='row clearfix'>
								<div class='row-3'>".$user_FIO[0]["surname_user"]." ".$user_FIO[0]["name_user"]." ".$user_FIO[0]["patronymic_user"]."</div>
								<div class='row-1'><input type='text' class='textarea-chat' name='new_ocenka[".$data[$i][0]["id_student"]."]' placeholder='Оценка ВКР' value='".$data[$i][0]["evaluation_vkr"]."' ></div>	
								</div>";
							}
